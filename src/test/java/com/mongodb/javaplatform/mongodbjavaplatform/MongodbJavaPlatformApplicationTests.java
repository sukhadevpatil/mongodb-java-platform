...
class MongodbJavaPlatformApplicationTests {
    ...
    void insertOneRecord() {
        ...
        Document document = new Document("_id", new ObjectId())
                ...
                .append("date",
						Date.from(LocalDate.of(2015, 2, 20).atStartOfDay(ZoneId.systemDefault()).toInstant()))
	...
	void testQueryingCollection() {
		...
		List<Document> documents = collection.find(
                and(eq("status", "PUBLISH"), gte("pageCount", 400), in("categories", "Java")))
                .into(new ArrayList<>());
